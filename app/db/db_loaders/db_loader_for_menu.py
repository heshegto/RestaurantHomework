...
class MenuLoader:
    ...
    def get_all_menus(self, db: Session, cache: Redis) -> list[dict]:
        keyword = 'menus'
    ...
    def get_one_menu(self, menu_id: UUID, db: Session, cache: Redis) -> list[dict] | None:
    ...
    def create_menu(self, menu: schemas.MenuCreate, db: Session, cache: Redis) -> list[dict] | None:
        keyword = ['menus']
        cache_database.delete_cache(keyword, cache=cache)
        return self.crud.create_item(menu, db).first()

    def update_menu(self, menu_id: UUID, menu: schemas.MenuCreate, db: Session, cache: Redis) -> list[dict] | None:
    ...
    def delete_menu(self, menu_id: UUID, db: Session, cache: Redis) -> list[dict] | None:
